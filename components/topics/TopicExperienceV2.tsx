/**
 * TopicExperienceV2 - Enhanced Topic Experience Component
 * 
 * Improvements over V1:
 * - Better progressive loading states
 * - Conceptual variants display support
 * - Authority level indicators for sources
 * - Preserved all existing features (charts, focus mode, etc.)
 * 
 * @version 2.0.0
 * @since Phase 2 - Scholarly Authority System
 */
'use client';
...
import { Topic, Source, Citation, TopicRelationship, ConceptualVariant } from '@/lib/types';
...
import { ConceptVariantsView } from '@/components/topics/variants/ConceptVariantsView';

// Types
type SectionType = 'definition' | 'overview' | 'article' | 'mashal' | 'personal_nimshal' | 'global_nimshal' | 'historical_context' | 'charts' | 'confusions' | 'variants' | 'sources';
...
export function TopicExperienceV2({ topic, relatedTopics, sources, citations, inlineCitations }: TopicExperienceProps) {
    ...
    const [conceptualVariants, setConceptualVariants] = useState<ConceptualVariant[]>([]);
    ...
    const sections: ArticleSection[] = [
        ...
        // Scholarly variants section (hidden unless data exists)
        ...(conceptualVariants && conceptualVariants.length > 0 ? [{
            type: 'variants' as SectionType,
            order: 10,
            content: (
                <ConceptVariantsView
                    variants={conceptualVariants}
                    sources={sources}
                />
            ) as unknown as string
        }] : [])
