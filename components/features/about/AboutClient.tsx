...
import { BookOpen, Search, Heart, Sparkles, Layers, Loader2 } from 'lucide-react';
...
export function AboutClient({ settings }: AboutClientProps) {
    return (
        <>
            <GlobalNav />
            <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-primary">
                ...
                >
                    ...
                    >
                        {settings?.about_title || 'Your Torah Thinking Space'}
                    </motion.h1>
                    {settings?.about_content ? (
                        <motion.div
                            variants={staggerItem}
                            className="mx-auto max-w-3xl text-left prose prose-lg dark:prose-invert"
                ...
                {/* Mission Section - Show only if no dynamic content or as secondary info */}
                {!settings?.about_content && (
                    <motion.section
