 ...
 * Structure: Collection → Volume (39) → Sicha (1509)
 */
...
export const config: SeferImportConfig = {
  ...
  hierarchy: {
    ...
    leafLevel: {
      ...
      mappings: {
        ...
        pageCount: 'pages',
        parsha: 'parsha',
        language: 'language', // Will be mapped: אידיש → yi, עברית → he
        metadata: {
          filename: 'filename',
          chelek: 'chelek',
        },
  ...
  transforms: {
    // Flatten the sichos_by_parsha structure
    preProcess: (data) => {
      const flatSichos: any[] = [];
